...
tab1, tab2, tab3, tab4 = st.tabs(["🏠 Locatie Beoordelen", "📊 Locaties Vergelijken", "➕ Locatie Toevoegen", "🔍 Locatie Zoeken"])
...
with tab1:
    ...
    if st.session_state.df.empty:
        ...
        st.subheader("🏗️ Bouwlocatie Beoordeling")
        ...
        with st.expander("🏙️ Ruimtelijke Inpassing", expanded=True):
            ...
            if bestemmingsplan == "Nee (niet passend)":
                ...
                ruimtelijke_score = round((bestemmingsplan_score + kadastraal_score + infra_score) / 3)
        ...
        st.session_state.df.at[loc_index, "Ruimtelijke Score"] = ruimtelijke_score
        st.session_state.df.at[loc_index, "Milieu Score"] = milieu_score
        st.session_state.df.at[loc_index, "Veiligheid Techniek Score"] = veiligheid_techniek_score
        st.session_state.df.at[loc_index, "Bereikbaarheid Score"] = bereikbaarheid_score
...
with st.sidebar:
    ...
    if 'loc_select' in st.session_state:
        loc_data = st.session_state.df[st.session_state.df["Locatie"] == st.session_state.loc_select].iloc[0]
        scores = [loc_data[c] for c in SCORE_LEGEND.keys()]
        totaal = sum(scores)
        max_score = len(scores) * 5
        percentage = totaal / max_score
        gemiddelde = totaal / len(scores)
        
        # Verbeterde kleurbepaling
        def get_gradient_color(percentage):
            r = int(255 * (1 - percentage**0.7))
            g = int(255 * percentage**1.3)
            b = 40
        ...
        # Eerst de scoreverdeling string maken
        score_distribution = ', '.join([f'{k.split()[0]}: {v}/5' for k,v in zip(SCORE_LEGEND.keys(), scores)])
        
        # Cirkel met animatie
        st.markdown(f"""
        <div style="margin: 0 auto; width: {circle_size}px; height: {circle_size}px; position: relative;"
             title="Scoreverdeling: {score_distribution}">
       ...
       # Metrics in vakjes met dynamische kleur
        col1, col2 = st.columns(2)
        with col1:
            st.markdown(f"""
        ...
        # Emoji breakdown (light versie van optie 4)
        st.markdown("---")
        ...
        for criterium in SCORE_LEGEND.keys():
            ...
            <div style="display: flex; align-items: center; 
                ...
                <div style="font-size: 18px; width: 30px;">{EMOJI_SCALE[score]}</div>
                <div style="flex: 1; font-size: 13px;">{criterium.split()[0]}</div>
                <div style="font-weight: bold; color: {SCORE_COLORS[score]}; 
